...
function memoize(func) {
  ...
  return function cacheFunction(...args) {
  ...
  return function retrier() {
    for (let i = 0; i < attempts; i += 1) {
      try {
        ...
        // If an error is thrown, retry until the attempts are exhausted
      }
    }
...
function logger(func, logFunc) {
  return function returnLogger(...args) {
    const functionName = func.name || 'anonymous';
    const argsString = args.map((arg) => JSON.stringify(arg)).join(',');
    const logStart = `${functionName}(${argsString}) starts`;
    const logEnd = `${functionName}(${argsString}) ends`;
